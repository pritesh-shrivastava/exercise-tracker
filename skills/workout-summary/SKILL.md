...
- `summary.py` shows the last 5 recent days only (grouped with body-part labels) — not a weekly breakdown. For weekly volume, query the DB directly or extend `fetch_summary()` in `tracker/core.py`.
