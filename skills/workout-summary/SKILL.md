...
description: Use when the user asks for a summary, stats, progress, recent workouts, personal records, PRs, this week's training, best lifts, coaching, what to train next, or how their training is going.
version: 1.2.0
---
...
- "coach me", "what should I train next", "next workout", "what should I focus on"
...
### Coaching/advisory summary
```
cd /home/azureuser/exercise-tracker && uv run python scripts/summary.py --coach
```
Returns: DB-backed advisory prompts for Telegram: recent body-part coverage, suggested next focus, and stale high-rep weighted PRs that may be ready for a small weight increase.
...
This applies to standard summary commands: `uv run python scripts/summary.py`, `uv run python scripts/summary.py --prs`, and `uv run python scripts/summary.py --coach`.

For coaching requests, first paste the raw `--coach` output in a code block. If the user explicitly asks for advice, interpretation, or "what should I do", add at most 3 short advisory bullets after the code block. Keep advice grounded in the report; do not invent exercises, weights, or injury guidance.
...
- `scripts/summary.py --coach` is advisory, not a logging path. If the user pastes workout lines, direct them to the private form instead of parsing or inserting rows.
- Coaching must use SQLite-backed command output. Do not use Hermes memory as a source of truth.
- Avoid medical/injury diagnosis. If soreness, pain, or recovery risk comes up, keep advice conservative and suggest reducing load or seeking qualified help when appropriate.
...
The private web form has a `PRs` page that renders the same DB-backed report path as `scripts/summary.py --prs`. Use that page for routine phone review. Use this skill when the user asks for PRs, summaries, or coaching in Telegram.
