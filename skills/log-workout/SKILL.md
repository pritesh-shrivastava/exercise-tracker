...
5. **When showing PRs to the user, the emoji-heavy raw output may not render in Telegram.** If the user says they can't see it ("Cant see"), reformat without emojis using plain section headers and `code` blocks. Don't repeat the raw emoji output — go straight to a clean format.
...
- **Suffix name drift creates fork.** Input like "Lateral Raise Dumbell" vs existing "Lateral Raise" — the parser doesn't normalize away the equipment suffix, creating a second entry in PR output. Same for "Seated Row" vs "Seated Row machine" (already seen in DB). Fix: merge into the canonical (shorter, older) name via SQL. Check if both variants already exist before logging by scanning PR output for near-duplicates.
- **New exercise name that should merge with existing canonical.** When a new variant appears (e.g. "Dumbbell Incline Chest Press" when "Dumbbell Bench Press" with `variation=incline` already exists), merge into the existing canonical name + variation. Check the PR output for similar exercises before choosing a name. The user expects entries to fold into the existing exercise, not fork. When the user says "rename it to X so it merges", follow the instruction immediately.
